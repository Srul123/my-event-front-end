...
const ChangeLanguageSelector: React.FC<Props> = ({ setMuiThemeLocal }) => {
  const dispatch = useDispatch();
  const currentLanguageCode = cookies.get("i18next") || defaultLanguage;
  const currentLanguage: Local =
  ...
  const handleChangeLanguage = (langCode: string | undefined) => {
    if (langCode === "he") {
      setMuiThemeLocal("heIL");
    } else {
      setMuiThemeLocal("enUS");
    }
        ...
        {languages.map((lang) => (
          ...
          >
            ...
            >
              ...
              <span
                style={{ margin: "0 1vw" }}
                className={`flag-icon flag-icon-${lang.countryCode}`}
              >
                {" "}
              </span>
            </div>
